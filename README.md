...
用一个统一的小型 GPT workload，在 **Single GPU → DDP → FSDP → FSDP + activation checkpointing** 之间做公平对比，量化每种策略的**峰值显存、吞吐（tokens/s）、步耗时与扩展效率**，并用 **PyTorch Profiler** 解释背后的 NCCL 通信行为。
...
同一个 MiniGPT workload 在三种策略下运行，共用统一 Trainer 与 Benchmark Harness：
...
├── configs/            # gpt_small.yaml / ddp.yaml / fsdp.yaml
├── src/mini_llm/
│   ├── model.py        # MiniGPT
│   ├── data.py         # synthetic dataset
│   ├── trainer.py      # 统一 trainer（single/ddp/fsdp）
│   ├── train.py        # 训练入口
│   ├── utils.py
│   └── distributed/
│       ├── ddp.py
│       └── fsdp.py
├── scripts/            # train_single/ddp/fsdp.sh, benchmark.sh
...
该脚本依次运行 `single → ddp → fsdp → fsdp+ac`，把结果追加到
`benchmarks/results/benchmark.csv`，并生成三张图。
...
| CUDA | 12.1 |
| PyTorch | 2.3.0+cu121 |
| dtype | bf16（autocast / FSDP MixedPrecision） |
...
1. **显存-吞吐权衡清晰，没有免费午餐**：从 Single → DDP → FSDP → FSDP+AC，峰值显存单调下降（5.97 → 6.37 → 5.15 → 2.13 GB），吞吐也单调下降（52,595 → 39,785 → 23,378 → 18,171 tokens/s）。**省显存必然牺牲吞吐。**
...
5. **扩展效率均 < 100%，小模型不适合盲目上多卡**：多卡存在通信开销、负载不均与 kernel 启动开销。FSDP/分片策略的真正价值在于**能训练单卡放不下的更大模型**，而非提升小模型吞吐。若要观察 DDP 近似线性扩展或 FSDP 相对优势，需增大模型/批次使计算量足以掩盖通信。
...
- [docs/setup-and-debug-log.md](docs/setup-and-debug-log.md) — 完整操作过程与排错记录（环境搭建、pytest 段错误、YAML 科学计数法、FSDP API 等报错及修复方法）
...
- [ ] v0.2：DeepSpeed ZeRO-2 / ZeRO-3 对比
